<?php
session_start();
include 'php/db.php';
$result = $conn->query("SELECT * FROM jobs");
?>
<!DOCTYPE html>
<html>
<head>
    <title>Jobs - CampusHire</title>
    <link rel="stylesheet" href="css/style.css">
</head>
<body>
<nav class="navbar">
    <h2>CampusHire</h2>
    <ul>
        <?php if(isset($_SESSION['user_id'])): ?>
            <li><a href="index.html">Home</a></li>
            <li><a href="upcoming.html">Upcoming</a></li>
            <li><a href="about.html">About</a></li>
            <li><a href="jobs.php">Jobs</a></li>
            <li><a href="dashboard.php">Dashboard</a></li>
            <li><a class="btn" href="logout.php" style="background-color:red;">Logout</a></li>
        <?php else: ?>
            <li><a href="index.html">Home</a></li>
            <li><a href="upcoming.html">Upcoming</a></li>
            <li><a href="about.html">About</a></li>
            <li><a href="jobs.php">Jobs</a></li>
            <li><a href="login.html">Login</a></li>
            <li><a class="btn" href="register.html">Register</a></li>
        <?php endif; ?>
    </ul>
</nav>

<h2 style="text-align:center; color:white; margin-top:30px;">Available Jobs</h2>
<div style="display:flex; flex-wrap:wrap; justify-content:center; gap:20px; padding:30px;">
<?php if($result && $result->num_rows > 0): ?>
    <?php while($row = $result->fetch_assoc()): ?>
    <div class="card">
        <h3><?php echo $row['title']; ?></h3>
        <p><?php echo $row['company']; ?></p>
        <p><?php echo $row['location']; ?></p>
        <form method="POST" action="php/apply.php">
            <input type="hidden" name="job_id" value="<?php echo $row['id']; ?>">
            <button type="submit">Apply</button>
        </form>
    </div>
    <?php endwhile; ?>
<?php else: ?>
    <p style="color:white;">No jobs available right now. Ask admin to add some!</p>
<?php endif; ?>
</div>
</body>
</html>